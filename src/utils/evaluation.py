...
def average_displacement_error(coordinates_predicted, coordinates_gt, num_peds):
    ...
    Args:
      coordinates_predicted: tensor of shape [max_num_ped, pred_length, 2].
      ...
      num_peds: tensor of shape []. Number of pedestrians that are in the
        sequence.

    Returns:
      tensor of shape [] containing the average displacement error of the
    ...
    return ade / tf.cast(num_peds * coordinates_gt.shape[1], tf.float32)
...
def final_displacement_error(coordinates_predicted, coordinates_gt, num_peds):
    ...
    Returns:
      tensor of shape [] containing the final displacement error of the
        pedestrians that are in the sequence.
